...
from config import llm_fail_logger
...
# =============================================================================
from ocr_schema import O_A_S
OCR_ANNOTATION_SCHEMA = O_A_S
...
# - OR "Drug Name" | "Dosage" | "Strength/Package Size" | "Billing Unit" | "UM Type" | "Code 1"
# - OR "HCPCS/CPT Code" | "Drug Name" | "HCPCS/CPT Code Description" | "Coverage Level" | "Notes & Restrictions"
# - OR "Drug Class/Drug Name" | "Reference Brand Name" | "Brand Only/Generic Notes" | "Preferred Drug Status" | "PA Status" | "Step Therapy Requirements" | "Quantity Limit (QL)" | "QL Days"
# - OR "Drug name" | "Brand or Generic" | "Drug tier" | "Coverage rules or limits on use"
# - OR "Drug Name" | "Special Code" | "Tier Category"
...
# - SPECIAL TEMPLATE 1: If headers are "Drug Name", "Dosage", "Strength", "Billing Unit", "UM Type", "Code 1":
#     - This template has NO Tier column.
#     - "UM Type" and "Code 1" are Requirements columns.
# - SPECIAL TEMPLATE 2: If headers are "HCPCS/CPT Code", "Drug Name", "HCPCS/CPT Code Description", "Coverage Level", "Notes & Restrictions":
#     - "Coverage Level" maps to Tier.
#     - "Notes & Restrictions" maps to Requirements.
#     - "HCPCS/CPT Code Description" provides additional drug details.
# - SPECIAL TEMPLATE 3 (AHCCCS): If headers are "Drug Class/Drug Name", "Reference Brand Name", "Brand Only/Generic Notes", "Preferred Drug Status", "PA Status", "Step Therapy Requirements", "Quantity Limit (QL)", "QL Days":
#     - Drug Name = "Drug Class/Drug Name" + "Reference Brand Name"
#     - This template has no "Drug tier" column.
#     - Requirements = "PA Status" + "Step Therapy Requirements" + "Quantity Limit (QL)" + "QL Days"
# - SPECIAL TEMPLATE 4: If headers are "Drug name", "Brand or Generic", "Drug tier", "Coverage rules or limits on use":
#     - Tier = "Drug tier" ONLY. Ignore "Brand or Generic". If "Drug tier" column is missing, set Tier to NULL.
#     - Requirements = "Coverage rules or limits on use"
# - SPECIAL TEMPLATE 5: If headers are "Drug Name", "Special Code", "Tier Category":
#     - "Special Code" maps to 'requirements'.
#     - "Tier Category" maps to 'drug tier'. Extract ONLY the tier value (e.g., "3", "NC", "4") and IGNORE the category text (e.g., "ANTIVIRALS").
...
# SPECIAL CASE 1: If table has "HCPCS/CPT Code Description" column, you MUST concatenate "Drug Name" column value AND "HCPCS/CPT Code Description" column value here.
# SPECIAL CASE 2: If table has "Reference Brand Name" column, you MUST concatenate "Drug Class/Drug Name" column value AND "Reference Brand Name" column value here (e.g. "AMPHETAMINE... (ADDERALL XR)")."""
#                             },
...
def robust_json_repair(json_string: str):
    ...
    try:
        ...
        llm_fail_logger.error(f"FAILED_JSON_REPAIR:\n{json_string}\n" + "-"*80)
        return default_output
...
def _is_index_entry(item: dict) -> bool:
    ...
    REVISED: More conservative to avoid false positives on valid drug entries.
    """
    ...
    if str(tier).strip().isdigit():
        ...
        if tier_val > 6:  # Tiers are usually 1-6
            logger.debug(f"🚫 Index entry (tier > 6): {drug_name} (tier={tier_val})")
            return True
    ...
    if str(reqs).strip().isdigit():
        ...
        if req_val > 6:
            logger.debug(f"🚫 Index entry (req > 6): {drug_name} (req={req_val})")
            return True
    
    # Heuristic 3: Drug name ends with page number pattern
    if RE_PAGE_NUMBER_END.search(drug_name):
        logger.debug(f"🚫 Index entry (page number pattern): {drug_name}")
        return True
    
    # Heuristic 4: Drug name contains dot leaders (strong index indicator)
    if RE_DOT_LEADERS.search(drug_name):
        logger.debug(f"🚫 Index entry (dot leaders): {drug_name}")
        return True
    ...
    if match:
        ...
        if num > 10:  # Unlikely to be a valid part of drug name
            logger.debug(f"🚫 Index entry (trailing number > 10): {drug_name}")
            return True
    
    # Heuristic 6: Drug name is ONLY a drug name with NO dosage info
    # AND tier/requirements are BOTH missing
    # REVISED: Only flag as index if it's VERY suspicious (no dosage, no tier, no requirements)
    # AND the name is very short (< 15 chars) - most common drug names are longer
    # This is the MOST conservative heuristic to avoid false positives
    has_dosage = bool(RE_DOSAGE_FORM.search(drug_name))
    if not has_dosage and not tier and not reqs:
        # Only flag VERY short names as potential index entries
        # Names like "ASPIRIN", "ADVIL", "TYLENOL" are 6-7 chars but are valid drugs
        # So we need to be even more conservative - only flag if < 5 chars OR has special patterns
        if len(drug_name) > 3 and len(drug_name) < 5:
            # Very short names (4 chars) with no info are suspicious
            logger.debug(f"🚫 Index entry (very short name, no info): {drug_name}")
            return True
...
def _is_extracted_data_from_index_page(drug_table: List[dict]) -> bool:
    ...
    # NEW CHECK: If >50% have requirements (even without tiers), it's likely a valid drug page
    # This catches PDFs where requirements are present but tiers are in a different format
    has_requirements_count = 0
    for item in drug_table:
        if item.get("drug_requirements"):
            has_requirements_count += 1
    
    if has_requirements_count / total >= 0.50:
        logger.info(f"✅ VALID DRUG PAGE: {has_requirements_count}/{total} ({has_requirements_count/total*100:.1f}%) entries have requirements. NOT an index page.")
        return False
    ...
    # RULE 1: High percentage of obvious index entries
    if total > 0 and index_entries / total >= 0.25:  # Raised back to 25% to be less aggressive
        logger.info(f"🚫 INDEX PAGE DETECTED (Rule 1): {index_entries}/{total} ({index_entries/total*100:.1f}%) entries are index entries")
        return True
    
    # RULE 1.5: If majority have valid tier formats (1-6), this is NOT an index page
    ...
    # RULE 1.6: If majority of entries have ANY drug_tier value (including cost-based tiers like
    ...
    # REVISED: More conservative thresholds to avoid false positives
    # Only trigger if ALL THREE are very high (90%+)
    if (no_dosage_count / total >= 0.90 and  # Raised from 80% to 90%
        no_requirements_count / total >= 0.90 and  # Raised from 80% to 90%
        no_tier_count / total >= 0.90):  # Raised from 80% to 90%
        logger.info(f"🚫 INDEX PAGE DETECTED (Rule 3 - Uniformity): {no_dosage_count}/{total} ({no_dosage_count/total*100:.1f}%) no dosage, {no_requirements_count}/{total} ({no_requirements_count/total*100:.1f}%) no requirements, {no_tier_count}/{total} ({no_tier_count/total*100:.1f}%) no tier")
        return True

    # RULE 4: If >90% have no tier AND no requirements AND >70% have no dosage
    # REVISED: More conservative to avoid filtering valid pages
    if (no_tier_count / total >= 0.90 and 
        no_requirements_count / total >= 0.90 and
        no_dosage_count / total >= 0.70):
        logger.info(f"🚫 INDEX PAGE DETECTED (Rule 4 - Missing Everything): {no_tier_count}/{total} ({no_tier_count/total*100:.1f}%) no tier, {no_requirements_count}/{total} ({no_requirements_count/total*100:.1f}%) no requirements, {no_dosage_count}/{total} ({no_dosage_count/total*100:.1f}%) no dosage")
        return True

    # If we reach here, log why we're NOT filtering this page
    logger.debug(f"✅ VALID DRUG PAGE: Passed all index detection rules. Stats: {no_tier_count}/{total} no tier, {no_requirements_count}/{total} no requirements, {no_dosage_count}/{total} no dosage, {has_requirements_count}/{total} have requirements")
    return False
