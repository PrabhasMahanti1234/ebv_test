...
from coverage import (
    det_coverage_status, detect_prior_authorization,
    detect_step_therapy, normalize_drug_tier, infer_drug_tier_from_text
)
import threading
...
# Removed obsolete coverage functions - now using coverage.py module
# Functions removed: determine_coverage_status, determine_final_coverage_status,
# detect_prior_authorization, detect_step_therapy, normalize_drug_tier, infer_drug_tier_from_text
# These are now imported from coverage module at the top of this file
...
def clean_special_chars(cleaned_text, **kwargs):
    ...
    # Step 3: Remove $ signs (math mode)
    if not kwargs.get('keep_currency', False):
        cleaned_text = cleaned_text.replace('$', '')
    ...
    # Step 8: Remove leading/trailing punctuation and normalize spaces
    if kwargs.get('keep_currency', False):
        # Don't strip '$' if we want to keep currency
        cleaned_text = re.sub(r'^[,\s\{\}\\]+|[,\s\{\}\\]+$', '', cleaned_text)
    else:
        # Strip '$' along with other punctuation
        cleaned_text = re.sub(r'^[,\s\$\{\}\\]+|[,\s\$\{\}\\]+$', '', cleaned_text)
