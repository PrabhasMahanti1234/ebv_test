            ...
            if SKIP_INDEX_PAGES:
                ...
                if not is_index:
                    ...
                    if total_content_lines > 10:
                        # NEW: Safety check - if page contains strong drug form indicators, it is NOT an index
                        # Force keep pages with specific keywords even if they have dot leaders
                        drug_form_safe_list = [
                            'tablet', 'capsule', 'injection', 'solution', 'suspension', 
                            'cream', 'ointment', 'drops', 'inhaler', 'shampoo', 'lotion',
                            'patch', 'gel', 'kit', 'powder', 'chewable'
                        ]
                        
                        has_drug_keywords = any(f" {kw} " in text_lower for kw in drug_form_safe_list)
                        if has_drug_keywords:
                            logger.info(f"   ✅ Page {page_num}: Contains drug form keywords - keeping despite formatting")
                            filtered_pages.append(page_num)
                            continue
